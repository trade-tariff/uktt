  ...
  private
    ...
    def find_and_parse_multiple_included(name, records)
      records.map do |record|
        ...
        raise ParsingError, "Error finding relationship '#{name}': #{record.inspect}"
      end
    end
    ...
    def find_and_parse_included(name, id, type)
      ...
      return {} if found_resource.none?

      parse_resource(found_resource)
    rescue NoMethodError
      raise ParsingError, "Error finding relationship - '#{name}', '#{id}', '#{type}': #{found_resource.inspect}"
    end
