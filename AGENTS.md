...
UKTT is a Ruby gem that works with the UK Trade Tariff API. It fetches sections, chapters, headings, commodities, goods nomenclatures, monetary exchange rates, and quota definitions, and it can test local, production, and other Frontend API servers with real API calls.

## Working Rules

Keep scope tight. Prefer simple changes. Verify generated or AI-suggested claims against source code and tests. Use `rg` for code search. Follow existing patterns.
...
Follow `.github/pull_request_template.md`. Use conventional commits. Apply exactly one risk label.
